...
if (-not $BuildImage)
{
    if ($Claude)
    {
        ...
        if ($ClaudePrompt)
        {
            ...
            $promptArg = "-Prompt `"$ClaudePrompt`""
            Write-Host "Executing: ``docker run --rm --memory=12g $dockerArgsAsString $VolumeMappingsAsString -w $SourceDirName $ImageTag pwsh -Command `"& .\eng\RunClaude.ps1 $promptArg`"" -ForegroundColor Cyan
            docker run --rm --memory=12g $dockerArgs @VolumeMappings -w $SourceDirName $ImageTag pwsh -Command "& .\eng\RunClaude.ps1 $promptArg"
        }
        else
        {
            # Interactive mode
            Write-Host "Executing: ``docker run --rm --memory=12g $dockerArgsAsString $VolumeMappingsAsString -w $SourceDirName $ImageTag pwsh -Command `"& .\eng\RunClaude.ps1`"" -ForegroundColor Cyan
            docker run --rm --memory=12g $dockerArgs @VolumeMappings -w $SourceDirName $ImageTag pwsh -Command "& .\eng\RunClaude.ps1"
        }
