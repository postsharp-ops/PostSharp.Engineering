...
The generated `Cleanup Docker containers` step, which every Docker-using configuration already has and which
runs with `ExecutionMode.Always`, now also runs the command named by `BUILDAGENT_CLEANUP_SCRIPT`. Containers
are removed first, so nothing is still writing. Unset means no command, which is every Windows agent.
