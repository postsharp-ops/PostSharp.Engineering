...
# This file is auto-generated from src/PostSharp.Engineering.BuildTools/Resources/DockerBuild.ps1 in the
...
# The generated script that cleans the build agent, by its path inside the container. It is in the mounted
# repository, so the container reaches it at the same path the host does.
#
# The container is what runs it, rather than a step on the agent, because on a Unix agent it is the only thing that
...
function Get-CleanUpBuildAgentScript([string]$sourceDirectory)
{
    return "$sourceDirectory/$EngPath/CleanUpBuildAgent.ps1"
}
...
"@
    ...
    if (-not $NoInit -and -not $KeepInit)
    {
        ...
        # After the environment variables, because the clean-up reads NUGET_PACKAGES from the environment those
        # assignments establish, and before everything else, because nothing this container does may restore first.
        # -InContainer tells the script that its caller is root and therefore the last thing that can remove these
        # directories, so a survivor fails the build instead of being left to somebody else.
        ...
        {
            $cleanUpScript = ConvertTo-PowerShellLiteral ( Get-CleanUpBuildAgentScript $ContainerSourceDir )
            "# Delete the stale packages of this product before anything restores.`n" +
                    "& $cleanUpScript -InContainer`n" +
                    "if (`$LASTEXITCODE -ne 0) { exit `$LASTEXITCODE }`n`n"
        }
...
"@ | Set-Content -Path $initScript -Encoding UTF8
    ...
    if (-not $BuildImage)
    {
        ...
        if ($Test)
        {
            ...
            # The stale packages of this product go before the test command, in the same shell, because a test image is
            # chosen for the tool chain under test and is not required to carry PowerShell - so a test container cannot
            # run CleanUpBuildAgent.ps1 at all. The command is asked of that script rather than built here, so that the
            # list of packages and the way they are deleted stay in one place; it runs on the host, where PowerShell is
            # what is running this.
            #
            # A Windows test container gets nothing here: there the agent's own step removes them, because a Windows
            # container leaves what it writes into a bind mount deletable by the account the agent runs as.
            $testNuGetCacheCleanup = if ($IsUnix -and -not $NoNuGetCache)
            {
                & (Get-CleanUpBuildAgentScript $SourceDirName) -EmitTestCommandPrefix -NuGetCacheDirectory $nugetCacheDir
            }
            else
            {
                ""
